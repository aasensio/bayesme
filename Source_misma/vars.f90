...
implicit none
	...
	type modelo_type
		...
		integer :: stray_light_nlambda, optically_thin
	...
	type chain_analysis_type
		...
		real(kind=8) :: evidence, evidence_uncertainty, avg_lnL, Dkl
	...
	type transmission_filter_type
		integer :: nfilters, nlambda		
		real(kind=8), pointer :: lambda(:), transmission(:,:), lambda0(:)
	end type transmission_filter_type
	
	type(stokes_type) :: Observation, Emergent
	type(stokes_type), pointer :: Stokes_Syn(:)
	type(stokes_type), pointer :: Stokes_Syn_HR(:)
	...
	type(transmission_filter_type) :: filter_profiles
	
	integer :: what_to_do, number_of_components, idum, chain_maxlength, uniform_proposal_steps
	integer :: start_or_restart, uniform_B, uniform_thB, spectro_or_filter
