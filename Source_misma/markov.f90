...
!------------------------------------------------------------
	...
	real(kind=8) :: compute_likelihood, weight, factor, sigma_min, chi2, reduc
...
! Theoretical estimation of the alpha constant
		...
		flast = compute_likelihood(model,linea,Observation,inv,Stokes_Syn(1),physical,chi2,1.d0)
...
! Propose with B vector uniform (uniform in B^3 and cos(theta))
			call fill_model_uniformvector(inv,stat,proposed)
...
! of the chain to allow a better mixing of the chain
			...
			fproposed = compute_likelihood(proposed,linea,Observation,inv,&
				Stokes_Syn(1),physical,chi2,sigma_factor)
