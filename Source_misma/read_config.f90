...
!--------------------------------------------------
	...
	integer :: ios, nargs
	integer :: use_stray_light, ind_par
...
! Read if we are doing spectroscopy or filterpolarimetry
		call lb(12,2)		
		read(12,*) spectro_or_filter
...
! Read the position and transmission of the filters
		if (spectro_or_filter == 1) then
			...
			call lb(12,1)
			read(12,*) file
			print *, 'Reading transmission filters...'
			open(unit=13,file=file,action='read',status='old')
			call lb(13,2)
			read(13,*) t1, t2
			if (t1 /= observed%nlambda) then
				print *, 'Number of filters is not consistent with observations...'
				stop
			endif
			
			filter_profiles%nlambda	= t2
			filter_profiles%nfilters = t1
			
			print *, 'Number of filters : ', filter_profiles%nfilters
			print *, 'Number of wavelengths for filters : ', filter_profiles%nlambda

			allocate(filter_profiles%lambda(filter_profiles%nlambda))
			allocate(filter_profiles%lambda0(filter_profiles%nfilters))
			allocate(filter_profiles%transmission(filter_profiles%nfilters,filter_profiles%nlambda))			

			call lb(13,1)
			read(13,*) (filter_profiles%lambda0(j),j=1,filter_profiles%nfilters)
			print *, 'Filter positions : '
			print *, filter_profiles%lambda0

			call lb(13,1)
			do i = 1, filter_profiles%nlambda			
...
! OPTICALLY THIN OR NOT
			read(12,*) modelo(i)%optically_thin

			if (modelo(i)%optically_thin /= 0 .and. number_of_components == 1) then
				print *, 'The presence of optically thin components requires at least 2 components'
				stop
			endif

			print *, '  ---------------------------------'
			print *, '   COMPONENT  ', i
			if (modelo(i)%optically_thin == 1) then
				print *, '   Optically thin component associated to component ', modelo(i)%optically_thin
			endif
			print *, '  ---------------------------------'
...
! DAMPING
			...
			if (modelo(i)%optically_thin == 0) then
! BETA
				call lb(12,1)		
...
! Test that all filling factors of macroscopic atmospheres
! add up to 1			
			if (modelo(i)%optically_thin == 0) then
...
! Read stray light profile
		if (use_stray_light > 0) then			
			...
			read(13,*) modelo(use_stray_light)%stray_light_nlambda
			...
			do i = 1, modelo(use_stray_light)%stray_light_nlambda
				read(13,*) t1, (modelo(use_stray_light)%straylight(j,i),j=1,4)
			enddo		
			close(13)
		endif
