...
! and put it into Cube(nest_npar)
			...
			do i = 1, number_of_components
				...
				if (model(i)%optically_thin == 0) then
		...
		slhood = compute_likelihood(proposed,linea,Observation,inversion,Stokes_Syn(1),physical,chi2,1.d0)
...
! Save MAP profile
		...
		write(12,*) maxlhood, chi2_min
 		...
 		if (spectro_or_filter == 1) then
 			open(unit=12,file=trim(adjustl(fich_chain))//'.stokes_map_HR',&
 			action='write',status='replace')
			do i = 1, Stokes_Syn_HR(1)%nlambda
				write(12,FMT='(F10.4,2X,4(E12.4))') Stokes_Syn_HR(1)%lambda(i),&
					Stokes_Syn_HR(1)%stokes(1,i), Stokes_Syn_HR(1)%stokes(2,i), &
					Stokes_Syn_HR(1)%stokes(3,i), Stokes_Syn_HR(1)%stokes(4,i)
			enddo
			close(12)
 		endif
...
! Analyze chains
		...
		write(*,*)
 		...
 		print *, 'Reduced minimum chi^2 = ', chi2_min / (4.d0*Stokes_Syn(1)%nlambda)
