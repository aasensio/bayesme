!********************************************************
! Convolution module for applying the macroturbulence
!********************************************************
module convol
implicit none
contains
...
real*8, allocatable :: signal_complex(:), filter_complex(:), ans(:)
real*8 :: filter_completo(size(signal)), a, b, c, d
integer :: m, n, n2, i
...
! !-------------------------------------------------------------------
! ! Return the Stokes profiles convolved with the macroturbulence
! !-------------------------------------------------------------------
! subroutine convolve_macroturbulence(dw,emerg,emerg_conv)
! real*8 :: dw(:), emerg(:), emerg_conv(:)
! real*8, allocatable :: filter(:), ans(:), padding(:), emerg_piece(:), dw_piece(:)
! real*8 :: macro_A, x, wave
! integer :: i, j, l, m, n, n2, from, to
! 
! 		from = 0		
! 		do l = 1, n_spectral_regions
! 
! ! Select a piece of spectrum
! 			to = from + n_freqs_region(l)
! 			allocate(emerg_piece(n_freqs_region(l)))
! 			allocate(dw_piece(n_freqs_region(1)))
! 			emerg_piece = emerg(from+1:to)
! 			dw_piece = dw(from+1:to)
! 		
! 			n = size(dw)
! 			wave = (final_wavelength(l) + initial_wavelength(l)) / 2.d0
! 			macro_A = wave * (macroturbulence*1.d5) / PC
! 			print *, 'Macroturbulence : ',macro_A, ' A'
! 			m = 3 * macro_A / wavelength_step(l)
! 			m = 2 * m + 1
! 			allocate(filter(m))
! 			filter = 0.d0
! 
...
! 			do i = 1, m/2+1
! 				x = (i-1.d0)*wavelength_step(l) / macro_A
! 				filter(i) = exp(-x**2)
! 			enddo
! 
! 			do i = m/2+2, m
...
! 
! 
! 			padding(1:n) = emerg_piece(:)
! 			padding(n:n2) = padding(n)
! 			call convolve(padding,filter)
! 			
! 					
! 			emerg_conv(from+1:to) = padding(1:n)
! 			
! 			from = to	
! 			
...
! 			if (allocated(padding)) deallocate(padding)
! 			
! 			deallocate(emerg_piece)
! 			deallocate(dw_piece)
! 		enddo
! 
