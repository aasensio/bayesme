...
! If the parameter is not in the set of inverted
! parameters, return -1
!------------------------------------------------------------
	...
	character(len=10) :: name
		...
		open(unit=27,file=trim(adjustl(fich_chain))//'.params_inverted',&
			action='read',status='old')

		do i = 1, chain_analysis%nparams
			read(27,*) str_parameter
...
! Locate the index of the variables that enter into the function
				do j = 1, chain_analysis%posterior_fun(i)%n_variables
					...
					if (indx(j) /= -1) write(*,FMT='(A,I2,2X,A,A,I3)') '     Variable ', j,&
