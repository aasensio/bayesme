...
!-----------------------------------------------------------------	
	...
	type(line_type) :: linea(:)
	integer :: i, j, k, nfilters, nhighres, n_lineas, n
	...
	character(len=1) :: str
		...
		nfilters = filter_profiles%nfilters
		nhighres = filter_profiles%nlambda
		n = nhighres

		Stokes_Syn%nlambda = nfilters
		Stokes_Syn_HighRes%nlambda = nhighres
			
		if (.not.associated(Stokes_Syn%lambda)) allocate(Stokes_Syn%lambda(nfilters))
		if (.not.associated(Stokes_Syn%stokes)) allocate(Stokes_Syn%stokes(4,nfilters))

		if (.not.associated(Stokes_Syn_HighRes%lambda)) allocate(Stokes_Syn_HighRes%lambda(nhighres))
		if (.not.associated(Stokes_Syn_HighRes%stokes)) allocate(Stokes_Syn_HighRes%stokes(4,nhighres))
			
		Stokes_Syn%stokes = 0.d0
		Stokes_Syn_HighRes%stokes = 0.d0
		Stokes_Syn_HighRes%lambda = filter_profiles%lambda
...
! Or a standard model
				...
				do i = 1, n_lineas
					if (linea(i)%theory == 'ZE') then
						call zeeman_profile(Stokes_Syn_HighRes,model(j),linea(i),zeeman_voigt,zeeman_faraday)
					endif
					if (linea(i)%theory == 'HF') then
						call zeeman_profile_hyperfine(Stokes_Syn_HighRes,model(j),linea(i),zeeman_voigt,zeeman_faraday)
					endif
				...
				Stokes_Syn_HighRes%stokes = Stokes_Syn_HighRes%stokes + model(j)%filling_factor * stokes
		...
		do i = 1, 4
			do j = 1, nfilters
 				Stokes_Syn%stokes(i,j) = &
					int_tabulated(Stokes_Syn_HighRes%lambda, filter_profiles%transmission(j,:)*Stokes_Syn_HighRes%stokes(i,:))
			enddo
		enddo
