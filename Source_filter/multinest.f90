...
! Save MAP profile
		stat%parameters_proposed = stat%parameters_MAP
 		...
 		do i = 1, Stokes_Syn%nlambda
 			write(12,FMT='(F10.4,2X,4(E12.4))') filter_profiles%lambda0(i),&
 		...
 		open(unit=12,file=trim(adjustl(fich_chain))//'.stokes_map_hr',&
 			action='write',status='replace')
 		do i = 1, Stokes_Syn_HighRes%nlambda
 			write(12,FMT='(F10.4,2X,4(E12.4))') Stokes_Syn_HighRes%lambda(i),&
 				Stokes_Syn_HighRes%stokes(1,i), Stokes_Syn_HighRes%stokes(2,i), &
 				Stokes_Syn_HighRes%stokes(3,i), Stokes_Syn_HighRes%stokes(4,i)
 		enddo
 		close(12)
